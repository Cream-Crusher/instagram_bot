...
def creates_a_link_to_a_hubble_image(folder_name):
    for reference_number in range(1, 4):
        ...
        url = response
        url = 'https:'+url
        target_path = os.path.join(folder_name, 'api_hubblev{}.jpg' .format(
            reference_number))
        download_image(url, target_path)


def creates_a_link_to_a_SpaceX_image(folder_name):
    for reference_number in range(1, 4):
...
if __name__ == '__main__':
    ...
    #creates_a_link_to_a_hubble_image(folder_name)
    creates_a_link_to_a_SpaceX_image(folder_name)
